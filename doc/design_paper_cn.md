...
__保险：__ 希望给汽车购买保险的汽车所有者只需提供所有权 token 和注册 token 即可满足保险公司的要求。通过将 Token 自动与保险公司的标准相匹配，一旦通过验证，保险公司就能够通过向用户发送保险 Token 来换取用户付款。
...
与之前通过账户、信任主体和证件构建的链接不同，（通过区块链技术）Token 了构建不同提供者和服务之间的连接。

Token build the join between different providers and services, which are used to be built by accounts, trust and paperwork.

__Uber:__ 如果车主希望成为优步司机，她可以通过提供所有权 token，保险token和注册token轻松证明她的车辆已经满足了优步的要求。然后优步自动向她提供一个Ubertoken，根据车主的需要，可以用来让自己成为 Uber 司机或允许第三方成为 Uber 司机。这些流程都不需要手动验证或帐户创建。
...
token可以提供更灵活、甚至可编程的所有权及其交互使用，并且和原本中心化、基于账户的服务体系提供同样的功能。
...
__Self-Uber:__ 更进一步，车主可以跳过优步，直接将车租给陌生人。 她如果不想让她的汽车被一些随意的陌生租车者破坏。她可以将她的租车者限制在那些拥有“优良司机组织”颁发的 attestation（认证） token的人范围内。 承租人证明他们有这个 token，向车主支付一笔款项，在一个原子交易中获得一个临时token，允许他们能够解锁汽车并使用一段时间。 这是在没有创建帐户的情况下完成的，也不需要车主去提交大量文档手动验证完成。
...
__销售:__ 如果车主希望出售汽车，她只需要在任何相关网站上列出价格。 可以通过原子交易的方式，所有权 token 和付款在一笔交易中完成，确保买方和卖方都不会受到欺骗。新的所有者用 token 可以解锁汽车并驾车离开，甚至无需面对面地与原所有者会面。 新买家事先就知道汽车是否已经注册，并且通过简单的验证卖家钱包中的所有权 token 就能直到汽车是否正常在交管所注册，是否是卖家合法拥有。 一旦发生交换，卖家的token就会失效，她无法再操作汽车。 一旦交换发生，也可以自动取消保险单，并为卖家提供提前取消的退款。
